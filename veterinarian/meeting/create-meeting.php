...
function create_meeting($user, $to, $topic, $date, $time, $pass, $serviceID) {
    ...
    try {
		...
		$pass = $data->password;
		$meeting_id = $data->id;
		$db->save_meetings($user, $to, $topic, $meeting_id, $url, $pass, $date, $time, $serviceID);
    } catch(Exception $e) {
        if( 401 == $e->getCode() ) {
...
if($topic != '' && $date != '' && $start != ''){
	create_meeting($user, $to, $topic, $date, $time, $password, $serviceID);
}else{
