...
$dtime = date("H:i:s", strtotime($_POST['time']));
$cost = $cost = intval(preg_replace('/[^\d.]/', '', $_POST['cost']));

//INVOICE
$date = $_POST['date'];
$time = $_POST['time'];
$totalCost = number_format($cost);
...
$sql = "SELECT email FROM user_profile WHERE user_id = '$userID'";
$result = $conn->query($sql);
if($result -> num_rows > 0){
	$row = $result -> fetch_assoc();
	$email = $row['email'];
	$insert = $conn->prepare("INSERT INTO invoice(invoice_ref, appointment_id, user_id, fullname, petname, service_title, branch_name, date, time, service_cost)VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
	$insert -> bind_param("ssssssssss", $invoiceref, $id, $userID, $fullname, $petname, $service, $branch, $ddate, $dtime, $cost);
	if($insert->execute()){
		$update = $conn->prepare("UPDATE appointments SET payment_status = 'pending', status = 'done' WHERE id = ?");
		$update -> bind_param("s", $id);
		if($update->execute()){
			$url = web_root.'client/payments/invoice/view/?view='.$id;
			...
			$sqlnotify -> bind_param("sssssssssss", $id, $user, $userID, $category, $service, $icon, $url, $title, $ndate, $ntime, $notif_status);
			if($sqlnotify->execute()){
				echo 'success';
				appointment($fullname, $email, $petname, $date, $time, $service, $branch, $todayDate, $todayTime, $totalCost);
			}else{
				echo 'failed';
			}
		}
