...
<html lang="en">
	<head>
		<title>Payments | Vets at Work Veterinary Clinic</title>
				...
				<div class="content-header">
					<div class="container-fluid">
						<div class="row mb-2">
							...
							<?php
								$chk = "SELECT count(*) as total_pending FROM invoice WHERE payment_status = 'pending'";
								$res = $conn->query($chk);
								if($res -> num_rows > 0){
									$ro = $res -> fetch_assoc();
									if($ro['total_pending'] > 0){
										echo '
										<div class="col-sm-6">
											<a href="'.web_root.'veterinarian/payments/invoice" class="btn btn-warning float-right"><i class="fas fa-file-invoice"></i> &nbsp;Invoice <span class="badge badge-danger">'.$ro['total_pending'].'</span></a>
										</div>';
									}
								}
							?>
									...
									<div class="card-body">
										<div class="table-responsive">
											<table id="paymentList" class="table table-striped table-hover table-sm">
												...
												<tbody>
													<?php
														$sql = "SELECT billing.ukayra_id, billing.fullname, billing.date, billing.time, billing.description, billing.amount, billing.status, services.service_title FROM billing INNER JOIN services ON services.service_id = billing.services WHERE billing.category = 'service' AND billing.archive_status = '0'";
