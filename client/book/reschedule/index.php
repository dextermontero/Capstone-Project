...
<html lang="en">
	<head>
		<title>Reschedule Appointment | Vets at Work Veterinary</title>
	...
	<body class="hold-transition sidebar-mini layout-fixed">
								...
								<div class="row">
									<div class="col-lg-7 col-md-7 col-sm-7 col-xs-7 col-12">	
										<div class="form-group pull-right">
											...
											<select name="preferred_branch" class="form-control select2"id="preferred_branch" disabled>
												<?php
									...
									<div class="col-lg-5 col-md-5 col-sm-5 col-xs-5 col-12">
										<div class="container">
											...
											<div class="form-group pull-right">
												...
												<input type="text" class="form-control" name="pet_name" id="pet_name" value="<?php echo $_GET['pet']; ?>" disabled>
											</div>
											<div class="form-group pull-right">
												<label>Branch:</label>
												<?php
											...
											<div class="form-group pull-right">
												...
												<?php
													if(isset($_GET['serviceID'])){
														...
														$sql = "SELECT service_id, service_title FROM services WHERE service_id = '$serviceID' AND status = '1' AND archive_status = '0'";
														$result = $conn->query($sql);
														if($result -> num_rows > 0){
															$row = $result -> fetch_assoc();
													?>
														<input type="text" class="form-control service" id="<?php echo $row['service_id']; ?>" name="service" value="<?php echo $row['service_title']; ?>" disabled>
													<?php
														}
													}
												?>
																...
																if(!empty($bookings)){
																	...
																	foreach($branchTime as $ts){
																		if(!in_array($ts, $bookings)){
																			...
																			?>
																			<div class="col-lg-4 col-md-4 col-sm-4 col-xs-4 col-4 mb-2">
																				<button type="button" class="btn btn-danger btn-block btnClick" id="timeselector" value="<?php echo $ts; ?>"><?php echo $ts; ?></button>
																			</div>
																			<?php
																			}else{
																			?>
											...
											<button name="submit" type="submit" class="btn btn-info mt-3 btn-block" id="book_btn" disabled>Reschedule Appointment</button>
		...
		<script src="<?php echo web_root; ?>client/book/reschedule/js/reschedule.js"></script>
