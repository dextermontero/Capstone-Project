...
if($result -> num_rows > 0){
	...
	$sql = $conn->prepare("UPDATE appointments SET date = ?, timeslot = ?, status = ? WHERE id = ?");
	$sql->bind_param("ssss", $date, $time, $status, $id);
	$sqlc = $conn->prepare("UPDATE calendar_events SET start_event = ?, end_event = ? WHERE token = ?");
	$sqlc -> bind_param("sss", $cstart, $cend, $id);
...
function appointment($fullname, $email, $date, $time, $service, $branch, $branch_address, $status){
	...
	$mail->SetFrom("vawvetclinic.not.official@gmail.com", "VAW Vet Clinic Reschedule of Appointment");
	//$mail->AddReplyTo($email, $fullname);
	//$mail->AddCC("cc-recipient-email@domain", "cc-recipient-name");
	$mail->Subject = "Reschedule of Appointment";
	$message = '
	<html>
		<body>
			<div>
				<p>Greetings <b>'.$fullname.'</b>,</p>
				This is your rescheduled appointment details that you requested for Vets at Work Veterinary Clinic
				<br><br>
