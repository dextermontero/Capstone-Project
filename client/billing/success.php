...
if($result -> num_rows > 0){
	...
	if($row['category'] == 'reservation'){
		if($row['status'] == null || $row['status'] == ''){
			...
			if($results -> num_rows > 0){
				...
				if($status == "scheduled"){
					...
					$asql = $conn->prepare("UPDATE appointments SET status = 'scheduled' WHERE id = ?");
					$asql -> bind_param("s", $appointmentID);
					if($asql->execute()){
						$newStatus = "scheduled";
						$sqlbill = $conn->prepare("UPDATE billing SET status = 'paid' WHERE paymogo_id = ?");
		...
		if($row['status'] == null || $row['status'] == ''){
			$updateApp = $conn->prepare("UPDATE appointments SET payment_status = 'paid' WHERE id = ?");
			$updateApp -> bind_param("s", $appointmentID);
			if($updateApp->execute()){
				$updateInvoice = $conn->prepare("UPDATE invoice SET payment_status = 'paid' WHERE appointment_id = ?");
				$updateInvoice -> bind_param("s", $appointmentID);
				if($updateInvoice->execute()){
					$updateBill = $conn->prepare("UPDATE billing SET status = 'paid' WHERE appointment_id = ?");
					$updateBill -> bind_param("s", $appointmentID);
						...
						<div class="main-body">
 							<div class="row">								
								<div class="col-lg-12 col-12">
												...
												<div class="col-lg-6 col-12">
													...
													<table class="table table-borderedless table-striped">
														...
														<tbody align="center">
															...
															<tr>
																<td class="w-25">Reservation Amount</td>
													...
													<div class="row" <?php if($row['category'] != 'service'){ echo 'hidden';}?>>
														<div class="col-sm-6">
														</div>
														<div class="col-sm-6 float-right">
															<button type="submit" class="btn btn-success btn-block" id="savePDF" onclick="getPDF()" >Print</button>
														</div>
													</div>
													<?php
														}
									...
									<div class="row" >
										<div class="col-sm-4" style="width:100%;height:100%;position: absolute;top:0;left:0;z-index:-99999999999;">
											<div class="card canvas_div_pdf">
												<div class="card-body" style="background:#fff;">
													<?php
														$sql = "SELECT billing.date, billing.time, billing.amount, services.service_title, branch.name, billing.status, billing.mode_of_payment, invoice.petname, invoice.invoice_ref FROM billing INNER JOIN services ON services.service_id = billing.services LEFT JOIN branch ON branch.branch_id = billing.branch_id LEFT JOIN invoice ON invoice.appointment_id = billing.appointment_id WHERE billing.paymogo_id = '$payId' AND billing.user_id = '$user'";
														$result = $conn->query($sql);
														if($result -> num_rows > 0){
															$row = $result -> fetch_assoc();
													?>												
													<small>Receipt <span id="name_file">Invoice #<?php echo $row['invoice_ref'];?></span></small>
													<div class="dropdown-divider"></div>
													<p class="text-center h2">Vets at Work Veterinary</p>
													<span class="text-secondary text-center">Unit B/F Divino Amore Bldg., # 8 Holy Spirit Drive, Don Antonio Heights, Quezon City</span>
													<div class="dropdown-divider"></div>
													<h4 class="text-center"><i class="fas fa-file-invoice text-success fa-1x"></i>&nbsp; Invoice Paid</h3>
													<table class="table table-striped table-md">
														<tbody>
															<tr>
																<th>Date/Time</th>
																<td class="text-right"><?php echo date("F d, Y", strtotime($row['date'])) .' - '. date("g:i A", strtotime($row['time']));?></td>
															</tr>
															<tr>
																<th>Pet Name</th>
																<td class="text-right"><?php echo ucfirst($row['petname']);?></td>
															</tr>
															<tr>
																<th>Service</th>
																<td class="text-right"><?php echo ucfirst(strtolower($row['service_title']));?></td>
															</tr>
															<tr>
																<th>Branch</th>
															...
															<tr>
																...
																<td class="text-right"><?php echo $row['mode_of_payment'];?></td>
															</tr>
															<tr>
																<th>Payment Status</th>
																<td class="text-right text-success"><?php echo ucfirst($row['status']);?></td>
															</tr>
														</tbody>
													</table>
													<div class="dropdown-divider"></div>
													<div class="row mt-2">
														...
														<div class="col-sm-6 text-right">
															<b class="h4"><b>Total : ₱</b> <?php echo number_format($row['amount']);?></b>
														</div>
													</div>
													<?php
														}
