...
$adminname = ucfirst($firstname) .' '. ucfirst($lastname);
		...
		if(move_uploaded_file($_FILES['file']['tmp_name'],$path)){
			...
			if($a->execute()){
				...
				if($c -> num_rows > 0){
					$d = $c -> fetch_assoc();
					$uid = $d['uid'];
					$sql = $conn->prepare("INSERT INTO admin_profile(admin_id, firstname, middlename, lastname, photo, position, email, contact_number, address, gender, birthdate, place_bday, create_date)VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
					$sql -> bind_param("sssssssssssss", $uid, $firstname, $middlename, $lastname, $file, $roles, $email, $phone, $address, $gender, $birthdate, $place_birth, $date);
					if($sql->execute()){
						...
						if($r -> num_rows > 0){
							...
							$time = date("H:i:s");
							$activity = "<b>$fullname</b> adding new administrator at [Admin Name : $adminname]";
							...
							if($ss->execute()){
								sendPassword($email, $adminname, $genPassword, $sendCode);
								echo 'success';
							}
						}
					}else {
