...
if(isset($_POST['id'])){
	...
	if($result -> num_rows > 0){
		...
		$query = "SELECT photo, firstname, lastname FROM user_profile WHERE user_id = '$blog_id'";
		$result1 = $conn->query($query);
		if($result1 -> num_rows > 0) {
			$row1 = $result1 -> fetch_assoc();
			$title = ucfirst($row1['firstname']) .' '. ucfirst($row1['lastname']);
			$specific = $row1['photo'];
			
			if($specific == 'default.png'){
				
			}else{
				unlink("../../../dist/img/profiles/$specific");
			}
			$chk = $conn->prepare("DELETE FROM user_profile WHERE user_id = ?");
			$chk -> bind_param("s", $blog_id);
			$chk2 = $conn->prepare("DELETE FROM login_tbl WHERE uid = ?");
			$chk2 -> bind_param("s", $blog_id);			
			$chk1 = $conn->prepare("DELETE FROM archive WHERE archive_id = ?");
			$chk1 -> bind_param("s", $id);
			if($chk->execute() && $chk1->execute() && $chk2->execute()){
